...
import crunch.darwin.particles.*;
...
public class Commands {
  public static class ChangeParticle implements CommandExecutor {
    ...
    public CommandResult execute(CommandSource src, CommandContext args) throws CommandException {
      ...
      if (args.getOne("particle effect").isPresent()) {
        if (GetParticleFromString.get(
                args.getOne("particle effect").get().toString().toLowerCase(),
          ...
          src.sendMessage(
              Text.of(
                  DarwinParticlesMain.particlesDefault,
                  " That particle does not exist or is not enabled"));
  ...
  public static class getParticlesInChunk implements CommandExecutor {
    ...
    public CommandResult execute(CommandSource src, CommandContext args) throws CommandException {
      Player player = (Player) src;
      com.intellectualcrafters.plot.object.Location plotLoc =
      ...
      if (Plot.getPlot(plotLoc) != null) {
        ...
        if (DarwinParticlesMain.allPlotsWithParticles.containsKey(
            player.getLocation().getExtent().getName() + ":" + plot.getId().toString())) {

          PlotParticles pp =
              DarwinParticlesMain.allPlotsWithParticles.get(
                  player.getLocation().getExtent().getName() + ":" + plot.getId().toString());
          ...
          if (!contents.isEmpty() && contents != null) {
            for (Text toFormat : contents) {
              ...
              sendParticle
                  .append(Text.of(TextColors.AQUA, " [TP]"))
                  .onClick(
                      TextActions.runCommand(
                          "/pap teleport "
                              + toFormat.getChildren().get(2).toPlainSingle()
                              + " "
                              + toFormat.getChildren().get(4).toPlainSingle()
                              + " "
                              + toFormat.getChildren().get(6).toPlainSingle()))
                  .build();
              sendToPlayer.append(Text.of(sendParticle));
              sendParticle.removeAll();
              sendParticle
                  .append(Text.of(TextColors.DARK_RED, " [Remove] "))
                  .onClick(
                      TextActions.runCommand(
                          "/pap delete "
                              + toFormat.getChildren().get(2).toPlainSingle()
              ...
              sendParticle
                  .append(
                      Text.of(
                          TextColors.LIGHT_PURPLE, toFormat.getChildren().get(0).toPlainSingle()))
                  .build();
            ...
            player.sendMessage(
                Text.of(
                    "The chunk you are in does not currently have any particles loaded, wait 5 seconds then try again."));
          }
        } else {
          player.sendMessage(
              Text.of(
    ...
    public CommandResult execute(CommandSource src, CommandContext args) throws CommandException {
      ...
      if (Plot.getPlot(plotLoc) != null) {
        ...
        if (plot.isAdded(player.getUniqueId()) || player.hasPermission("plots.admin.build.other")) {
          ...
          if (allParticles.containsKey(loc.getChunkPosition())
              && allParticles.get(loc.getChunkPosition()).containsKey(loc)) {
            ...
            DarwinParticlesMain.allPlotsWithParticles.put(
                player.getLocation().getExtent().getName() + ":" + plot.getId().toString(), pp);
            String tableName;
            if (player.getLocation().getExtent().getName().toLowerCase().contains("plot")
                || player.getLocation().getExtent().getName().toLowerCase().contains("contest")) {
              tableName = "Plots";
            } else {
              ...
              System.out.println(
                  tableName
                      ...
                      + (loc.getBlockX() + "," + loc.getBlockY() + "," + loc.getBlockZ())
                          .toString());
              DarwinParticlesMain.db.removeFromDB(
                  tableName,
                  player.getLocation().getExtent().getName(),
                  plot.getId().toString(),
                  (loc.getX() + "," + loc.getY() + "," + loc.getZ()).toString());
            ...
            player.sendMessage(
                Text.of(
                    DarwinParticlesMain.particlesDefault,
                    "This particle does not exist in this plot."));
            return CommandResult.success();
          }
          ...
          player.sendMessage(
              Text.of(
                  DarwinParticlesMain.particlesDefault,
                  "You must be trusted or added to remove particles."));
          return CommandResult.success();
        }
      } else {
        player.sendMessage(
            Text.of(
                DarwinParticlesMain.particlesDefault,
                "You must be in the plot to delete particles."));
        return CommandResult.success();
      }
      return CommandResult.success();
    }
    ...
    public CommandResult execute(CommandSource src, CommandContext args) throws CommandException {
      ...
      if (Plot.getPlot(plotLoc) != null) {
        ...
        player.sendMessage(
            Text.of(
                DarwinParticlesMain.particlesDefault,
                "You must be in the plot to teleport to particles."));
        return CommandResult.success();
      }
      return CommandResult.success();
    }
  ...
  public static class MakeParticle implements CommandExecutor {
    ...
    public CommandResult execute(CommandSource src, CommandContext args) throws CommandException {
      ...
      Location loc =
          new Location(
              Sponge.getServer().getWorld(player.getLocation().getExtent().getName()).get(),
  ...
  public static class openGUI implements CommandExecutor {
    ...
    @Override
    public CommandResult execute(CommandSource src, CommandContext args) throws CommandException {
      if (src instanceof Player) new UserInterface((Player) src);
      return CommandResult.success();
    }
  }
}
